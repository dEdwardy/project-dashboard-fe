    ...
    <a-tabs lazy-load>
        ...
        <div text-left>
          ...
          <div ml-4 mt-4>
            ...
            <div ml-4 mt-4 border-2 p-4>
              ...
              <p>
                异步流程控制 并发上传切片 并控制并发数量6（p-limit/promise）
              </p>
            </div>
          </div>
          http
      ...
      <a-tab-pane key="2" title="2.RBAC权限设计">
        ...
        <div ml-4 mt-4 border-2 p-4>
          ...
          <p m-b-2>
            username + password(crypto) => backend => userinfo & token => local
            persist => before api send add token(header bearer token)
          </p>
          <p m-b-2>
            getRoutesInfo => routes => resolve routes addRoutes
          </p>
          <p m-b-2>
            axios request interceptor add token && response interceptor handle
            http status
          </p>
          <p>
            router hooks beforeEach 判断是否存在token，此外加上whiteList(for
            login register pages which does not need login)
          </p>
          <div m-b-4>
